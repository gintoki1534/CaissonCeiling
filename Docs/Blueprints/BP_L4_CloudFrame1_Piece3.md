# BP_L4_CloudFrame1_Piece3

## 1. 蓝图定位

本文档由 Docs/BlueprintExports/BP_L4_CloudFrame1_Piece3.json 自动整理生成，用于记录当前蓝图的真实导出结构。

## 2. 基础信息

1. 蓝图名：BP_L4_CloudFrame1_Piece3
1. 资源路径：/Script/Engine.Blueprint'/Game/UI/Level4/Pieces/BP_L4_CloudFrame1_Piece3.BP_L4_CloudFrame1_Piece3'
1. 蓝图类型：Blueprint
1. 父类：CaissonCeiling.Level4PuzzlePieceActor
1. 生成类：BP_L4_CloudFrame1_Piece3_C
...
### 8.1 BP_OnCorrectVisualChanged

1. 图类型：BlueprintGraph.EdGraphSchema_K2
1. 模块数：3
1. 连接数：3
1. 执行链：BP_OnCorrectVisualChanged -> SetVariableOnPersistentFrame -> ExecuteUbergraph_BP_L4_CloudFrame1_Piece3
1. 关键节点：
   - ExecuteUbergraph_BP_L4_CloudFrame1_Piece3
...
1. 模块数：3
1. 连接数：3
1. 执行链：BP_OnSelectedVisualChanged -> SetVariableOnPersistentFrame -> ExecuteUbergraph_BP_L4_CloudFrame1_Piece3
1. 关键节点：
   - ExecuteUbergraph_BP_L4_CloudFrame1_Piece3
     输入：execute <- SetVariableOnPersistentFrame:then
